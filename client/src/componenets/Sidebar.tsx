        ...
        <div className={`h-full sm:max-w-sm rounded-xl bg-gray-900 border-gray-800 transition-all ${isMenuOpen ? 'max-sm:w-0 overflow-hidden ' : 'w-full'} `}>
                        ...
                        if (isMessage) {
                            ...
                            return (
                                <div key={msg.id} className={`flex items-start gap-3 ${isUser ? "justify-end" : "justify-start"}`}>
                                    ...
                                    <div className={`${isUser ? "bg-linear-to-r from-indigo-500 to-indigo-600 text-white rounded-tr-none" : " rounded-tl-none bg-gray-800 text-gray-100"} max-w-[80%] p-2 px-4 rounded-2xl shadow-sm text-sm mt-5 leading-relaxed `}>
                                        {msg.content}
                                    </div>
                                    {isUser && (
                                ...
                                <div key={ver.id} className='w-4/5 mx-auto my-2 p-3 rounded-xl bg-gray-800 text-gray-100 shadow flex flex-col gap-2 '>
                                    ...
                                    <div className='flex items-center justify-between'>
                                        {project.current_version_index === ver.id ? (
                                            ...
                                            (
                                                <button onClick={()=>handleRollBack(ver.id)} className='px-3 py-1 rounded-md text-xs bg-indigo-500 hover:bg-indigo-600'>Roll Back to Prev Version</button>
                                            )}
          ...
          <div className="flex items-center gap-2">
            <textarea onChange={(e)=>setInput(e.target.value)} value={input}
             rows={4} placeholder='Describe your Website request Changes....' className='flex-1 p-3 rounded-xl resize-none text-sm outline-none ring ring-gray-700 focus:ring-indigo-500 bg-gray-800 text-gray-100 placeholder:text-gray-400 transition-all' 
